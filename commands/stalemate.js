...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('stalemate')
		.setDescription('Send an alternative win condition to players in a game that is stalemating')
		.addStringOption(option =>
			option.setName('wincondition1')
				.setDescription('What is the first alternative win condition?')
				.setRequired(true)
				.addChoices(
					{ name: 'Assassins', value: 'assassins' },
					{ name: 'First to kill', value: 'First to kill' },
					{ name: 'Capture 70%', value: 'Capture 70%' },
					{ name: 'Capture 60%', value: 'Capture 60%' },
					{ name: 'Capture 50%', value: 'Capture 50%' },
					{ name: 'First to hold a bonus', value: 'First to hold a bonus' },
					{ name: 'First to hit 50% (of any players troops)', value: 'First to hit 50%' },
					{ name: 'First to die', value: 'First to die' })
			)	
			.addStringOption(option =>
				option.setName('wincondition2')
					.setDescription('What is the second alternative win condition?')
					.setRequired(true)
					.addChoices(
						{ name: 'Assassins', value: 'assassins' },
						{ name: 'First to kill', value: 'First to kill' },
		...
		async execute(interaction) {
			...
			let errors = "Stalemate command executed.";
			let users = [];
			let targets = [];

			const winconditionA = interaction.options.getString('wincondition1');
			const winconditionB = interaction.options.getString('wincondition2');

			const wincondition = Math.random() < 0.5 ? winconditionA : winconditionB;
			...
			if (wincondition == "assassins") {
				...
				for (i = 0; i < users.length; i++) {
					...
					await users[i].send(`The alternative win condition for this game is: ${wincondition}`).catch(() => errors = errors + `\n${users[i].user.nickname} does not accept DMs. Unable to tell them the alternative win condition :(`);
