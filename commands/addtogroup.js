        ...
        try {
            ...
            if (result.length == 1) {
                ...
                if (group) {
                    ...
                    sql = "SELECT 1 FROM `" + global.config.mysql_database + "`.`eventmanager__events` e INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__staff` es ON e.`id` = es.`eventid` AND es.`userid` = " + interaction.user.id + " AND e.`id` = " + group.eventid + "";
                    staff = await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });
                    sql = "SELECT 1 FROM `" + global.config.mysql_database + "`.`eventmanager__events` e INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__admins` es ON e.`id` = es.`eventid` AND es.`userid` = " + interaction.user.id + " AND e.`id` = " + group.eventid + "";
                    ...
                    if (staff.length > 0 || admin.length > 0) {
                        ...
                        sql = "SELECT * FROM `" + global.config.mysql_database + "`.`eventmanager__signups` WHERE `eventid` = '" + group.eventid + "' AND `playerid` = '" + player.id + "' AND `validto` IS NULL";
                        ...
                        if (signups.length == 1) {
                            ...
                            if (signup) {
                                if (signup.bracket == group.bracketid) {
                                    sql = "SELECT * FROM `" + global.config.mysql_database + "`.`eventmanager__games` WHERE `roundid` = '" + group.roundid + "' AND `playerid` = '" + player.id + "' AND `validto` IS NULL";
                                    games = await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });
                                    if (games.length == 0) {

                                        const thread = await guild.channels.fetch(threadid);

                                        sql = "SELECT g.* FROM `" + global.config.mysql_database + "`.`eventmanager__groups` g INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__groupmembers` gm ON gm.`groupid` = g.`id` AND g.`roundid` = '" + group.roundid + "' AND gm.`playerid` = '" + player.id + "' AND gm.`validto` IS NULL";
                                        ...
                                        if (currentgroups.length == 1) {
                                            ...
                                            if (currentgroup) {
                                                sql = "UPDATE `" + global.config.mysql_database + "`.`eventmanager__groupmembers` SET `validto` = NOW() WHERE `validto` IS NULL AND `playerid` = " + player.id + " AND `groupid` = " + currentgroup.id + "";
                                        ...
                                        sql = "INSERT INTO `" + global.config.mysql_database + "`.`eventmanager__groupmembers` VALUES (NULL," + group.roundid + ", " + group.id + ", " + player.id + ", NOW(), NULL, NULL, NULL)";
                                        await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });
