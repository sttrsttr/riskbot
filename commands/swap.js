            ...
            if (result.length == 1) {
                ...
                if (event) {
                    ...
                    if (staff.length > 0 || admin.length > 0) {
                        ...
                        sql = "SELECT gm.`playerid` AS `from_player`, gm2.`playerid` AS `to_player`, g.`id` AS `from_group_id`, g.`name` AS `from_group_name`, g.`threadid` AS `from_group_threadid`, g2.`id` AS `to_group_id`, g2.`name` AS `to_group_name`, g2.`threadid` AS `to_group_threadid`, g.`roundid` FROM `" + global.config.mysql_database + "`.`eventmanager__events` e INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__rounds` er ON e.`id` = er.`eventid` INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__groups` g ON er.`id` = g.`roundid` AND e.`id` = " + event.id + " INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__groupmembers` gm ON gm.`groupid` = g.`id` AND gm.`playerid` = " + user1.id + " AND gm.`validto` IS NULL AND g.`completed` IS NULL INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__groups` g2 ON er.`id` = g2.`roundid` AND g2.`roundid` = g.`roundid` INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__groupmembers` gm2 ON gm2.`groupid` = g2.`id` AND gm2.`playerid` = " + user2.id + " AND gm2.`validto` IS NULL AND g2.`completed` IS NULL AND g.`id` != g2.`id` AND gm.`playerid` != gm2.`playerid`";
                        ...
                        if (groups.length != 1) {
                            ...
                            sql = "UPDATE `" + global.config.mysql_database + "`.`eventmanager__groupmembers` gm INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__groups` g ON gm.`groupid` = g.`id` INNER JOIN `" + global.config.mysql_database + "`.`eventmanager__rounds` er ON er.`id` = gm.`roundid` AND er.`eventid` = " + event.id + " SET gm.`validto` = NOW() WHERE gm.`validto` IS NULL AND g.`completed` IS NULL AND gm.`playerid` = " + swap.from_player + "";
                            await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });
                            sql = "INSERT INTO `" + global.config.mysql_database + "`.`eventmanager__groupmembers` VALUES (NULL," + swap.roundid + ", " + swap.to_group_id + ", " + swap.from_player + ", NOW(), NULL, NULL, NULL)";
                            await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });


                            sql = "INSERT INTO `" + global.config.mysql_database + "`.`eventmanager__playerlog` VALUES (NULL," + swap.from_player + "," + event.id + ",NOW(),'Initiated a swap into another group','Swap-command by staff',NULL,NULL)";
                            await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });

                            sql = "INSERT INTO `" + global.config.mysql_database + "`.`eventmanager__playerlog` VALUES (NULL," + swap.to_player + "," + event.id + ",NOW(),'Swapped with another player','Swap-command by staff',NULL,NULL)";
                            await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });
                            ...
                            sql = "INSERT INTO `" + global.config.mysql_database + "`.`eventmanager__groupmembers` VALUES (NULL," + swap.roundid + ", " + swap.from_group_id + ", " + swap.to_player + ", NOW(), NULL, NULL, NULL)";
                            await new Promise((resolve, reject) => { con.query(sql, function (err, result) { if (err) reject(err); resolve(result); }); });
